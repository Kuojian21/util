...
public class ElasticsearchTest2 {
	private static KjSnowflake kj = new KjSnowflake(2);
	...
	public static void createIndex() throws IOException {
		...
		DeleteIndex deleteIndex = new DeleteIndex.Builder("news2").build();
		...
		mapping.put("kj", type);
		CreateIndex createIndex = new CreateIndex.Builder("news2").mappings(mapping).build();
	...
	public static void batchInsert() throws IOException {
		...
		for (int i = 0; i < 10000; i++) {
			Bulk.Builder bulkBuilder = new Bulk.Builder().defaultIndex("news2").defaultType("kj");
			for (int j = 0; j < 100; j++) {
				News news = new News();
